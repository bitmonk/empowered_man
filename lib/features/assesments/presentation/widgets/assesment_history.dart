...
class _AssesmentHistoryState extends State<AssesmentHistory> {
  ...
  String? getEmptyMessage(bool isSearchActive) {
    if (!isSearchActive &&
        controller.getAssessmentHistoryState.value == TheStates.initial) {
      return 'Loading your assessment history...';
    }

    if (controller.getAssessmentHistoryState.value == TheStates.error) {
      return controller.searchError.value ?? 'Failed to load assessments.';
    }

    if (isSearchActive) {
      return 'No assessments found matching your search';
    }

    return 'No assessments available yet.';
  }
    ...
    return Obx(() {
      ...
      return Column(
        children: [
          ...
          const VerticalSpacing(12),
          if (!isSearchActive)
            controller.getAssessmentHistoryState.value.showWidget(
                loading: () => const Padding(
                      padding: EdgeInsets.symmetric(vertical: 50),
                      child: LoadingWidget(),
                    ),
                error: () => CustomErrorWidget(
                      verticlePadding: const EdgeInsets.only(top: 30),
          ...
          if (isSearchActive)
            controller.getAssessmentHistorySearchState.value.showWidget(
                initial: () => const CustomErrorWidget(
                            ...
                            builder: (context, constraints) {
                              return Column(
                                children: [
                                  ...
                                  if (controller.getAssessmentHistorySearchState
                                              .value ==
                                          TheStates.loading &&
                                      isSearchActive)
                                    const Padding(
                                  ...
                                  if ((isSearchActive &&
                                          ...
                                          !controller
                                              .assessmentHistoryPaginationPageController[
                                                  AssessmentHistoryPagination
                                                      .search]!
                                              .isInitialLoading
                                              .value) ||
                                      (!isSearchActive &&
                                          controller.getAssessmentHistoryState
                                                  .value ==
                                              TheStates.loading &&
                                          !controller
                                              .assessmentHistoryPaginationPageController[
